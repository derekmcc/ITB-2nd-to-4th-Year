...
public class FuzzyAirconController extends PApplet{
    ...
    public void createFuzzyEngine(){
        ...
        inputVariable1.addTerm(new Trapezoid("baltic", 0, 0, 5, 10));
        inputVariable1.addTerm(new Triangle("bitter", 5, 10, 15));
        inputVariable1.addTerm(new Triangle("warm", 10, 15, 20));
        inputVariable1.addTerm(new Triangle("hot", 15, 20, 25));
        inputVariable1.addTerm(new Triangle("scorching", 20, 25, 35));
        inputVariable1.addTerm(new Trapezoid("sweltering", 25, 35, 40, 40));
        //----------------------------------------------------------------------------------------------
        ...
        inputVariable2.addTerm(new Trapezoid("baltic", 0, 0, 5, 10));
        inputVariable2.addTerm(new Triangle("bitter", 5, 10, 15));
        inputVariable2.addTerm(new Triangle("warm", 10, 15, 20));
        inputVariable2.addTerm(new Triangle("hot", 15, 20, 25));
        inputVariable2.addTerm(new Triangle("scorching", 20, 25, 30));
        inputVariable2.addTerm(new Trapezoid("sweltering", 25, 30, 40, 40));
        //----------------------------------------------------------------------------------------------
        ...
        outputVariable.addTerm(new Trapezoid("cool", -10.00, -10.00, -5.00, -2.5));
        outputVariable.addTerm(new Trapezoid("nochange", -5.00, -2.5, 2.5, 5.00));
        outputVariable.addTerm(new Trapezoid("heat", 2.5, 5.00, 10.00, 10.00));
        //----------------------------------------------------------------------------------------------
        ...
        ruleBlock.addRule(Rule.parse("if (room is baltic) and (target is baltic) then command is nochange", engine));
        ruleBlock.addRule(Rule.parse("if (room is bitter) and (target is bitter) then command is nochange", engine));
        ...
        ruleBlock.addRule(Rule.parse("if (room is scorching) and (target is scorching) then command is nochange", engine));
        ruleBlock.addRule(Rule.parse("if (room is sweltering) and (target is sweltering) then command is nochange", engine));
         
        ruleBlock.addRule(Rule.parse("if (room is baltic or room is bitter) and (target is warm) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is baltic or room is bitter) and (target is hot) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is baltic or room is bitter) and (target is scorching) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is baltic or room is bitter) and (target is sweltering) then command is heat", engine));
        
        ruleBlock.addRule(Rule.parse("if (room is bitter or room is warm or room is hot or room is scorching or room is sweltering) and (target is baltic) then command is cool", engine));
        
        ruleBlock.addRule(Rule.parse("if (room is baltic) and (target is bitter) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is bitter) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is bitter) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is scorching) and (target is bitter) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is sweltering) and (target is bitter) then command is cool", engine));
        
        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is warm) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is scorching) and (target is warm) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is sweltering) and (target is warm) then command is cool", engine));
        
        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is hot) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is scorching) and (target is hot) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is sweltering) and (target is hot) then command is cool", engine));

        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is scorching) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is scorching) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is sweltering) and (target is scorching) then command is cool", engine));
        
        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is sweltering) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is sweltering) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is scorching) and (target is sweltering) then command is heat", engine));
